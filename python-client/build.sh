...
export CIBW_TEST_COMMAND='pytest {project}/python-client/tests -s'
...
export CIBW_BEFORE_TEST="\
  ...
  rustup show &&\
  PIPENV_PIPFILE=./python-client/Pipfile pipenv lock -r > requirements.txt &&\
...
export CIBW_BEFORE_BUILD_LINUX="\
  ...
  PIPENV_PIPFILE=./python-client/Pipfile pipenv lock -r > requirements.txt &&\
  pip install -U -r requirements.txt &&\
  unlink python-client/disco-src &&\
  cp -r disco python-client/disco-src &&\
  cp -r proto python-client/proto &&\
  tree -I 'node_modules|target|build' python-client"
